############################################
####script to optimize a quadratic discontinuous objective functional by
...
class GradientDescent:
    def __init__(self, xstart, sigma=1.0, npop=10, gradient_available = False, check_for_stability = False, use_one_directional_smoothing= False,npop_der = 0, npop_stoch = 0):
        ...
        self.softmax_der = lambda a,b: (a+b+np.sqrt((a-b)**2 +0.001))/2
    ...
    def generate_samples(self):
        ...
        if self.use_one_directional_smoothing:
            ...
            base_2d = np.linspace(-1,1,int(np.sqrt(self.npop))+2)[1:-1]
    ...
    def optimize(self, objective_function, iterations=100, gradient_function = None, args=()):
        iteration = 0
...
if __name__ == "__main__":
    ...
    def parabola_gradient(x, args=()):
        ### gradient function for discontinuous parabola
        x = np.reshape(x, (-1, x[0].shape[-1]))
    ...
    iter_max = 150 ### max number of optim steps
    multi = 10 ### multiplication factor to balance number of func evals between methods
    max_try = 100 ### max number of random runs


    ##### for the example run set max try to 1
##    iter_max = 150 ### max number of optim steps
##    multi = 10 ### multiplication factor to balance number of func evals between methods
##    max_try = 1 ### max number of random runs

    
    pop_per_dir = 10
    sigma = 1*1.0 ### size of variance for normal distribution area for sampling



    ## folder name
##    date_time = 'final_new' ### for run from 100 starting points
##    date_time = 'final_exrun_new' #### for example run
    date_time = 'test'
    
    pathing = './Ex1_' + date_time
...
##        xstart=[9,9] ### deterministic point used in example run
        ### for random runs
        xstart = np.random.rand(2)*20-10
        ...
        ### read out results of number of func evals each iteration
