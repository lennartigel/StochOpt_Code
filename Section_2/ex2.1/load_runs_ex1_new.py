############################################
#### Script to load the generated data from your run of the gd_ex1.py script
...
date_time = 'final_new' ### where dataset is stored
...
if do_work:
    ...
    y1_store = np.zeros(((iter_max,max_try)))
    ...
    opt1 = GradientDescent( xstart = [-1,1], npop = 100, sigma = 1, gradient_available = False)
...
ax4[-1].set_ylim([lower_lim-0.2, upper_lim+0.2])
ax4[-1].set_ylabel("ASM", fontsize = 12)


ax4[0].set_xscale("log")
ax4[0].set_ylim([lower_lim-0.2, upper_lim+0.2])
