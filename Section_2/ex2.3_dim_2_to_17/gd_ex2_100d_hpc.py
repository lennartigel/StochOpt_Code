...
##    pathing = './StochOpts/comp_small_new/final_2_to_17_new_' + str(ndim)
    pathing = './StochOpts/comp_small_new/test' + str(ndim)
    ...
    for i in range(max_try):
        ...
        ### start optimization
        print('##########################Optimizating Stochastic Step %i dim: %i ##########################'%(i,ndim))
        opt = GradientDescent(
            xstart=xstart, npop=pop_per_dir_stoch**ndim, sigma=sigma, gradient_available = False, npop_stoch = pop_per_dir_stoch
        )
