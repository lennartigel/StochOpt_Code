...
## 1.Gradient_Descent_variable_circle.py
...
| num_id_funcs | ASM only; number of level set functions to be considered | 
...
Performs optimization runs from random starting points in the domain "[-20,20]^2". 
...
- **do_work**: Set to True to process output data from "gd_ex_nGs.py" and store the following output in "Vals_hybrid.npy" and "Vals_stoch.npy" for ASM and GSM, respectively
...
2. Set general parameters in "gd_ex_nGs.py".
3. Run "gd_ex_nGs.py".
4. Use "load_runs_ex_nGs.py" to compute statistics and generate plots for specified dataset.
5. Use "load_landscape_ex_nGs.py" to visualize landscapes for specified dataset.

The default folder in "gd_ex_nGs.py" is set to 'test', so nothing is overwritten by accident. 
The default value "do_work=False" in "load_runs_ex_nGs.py", so nothing is accidentally overwritten. Change it to generate data for specific data sets and landscapes.
