...
class SpringFracture:
    def __init__(self, L, B, n_time_steps, ntnn = False, endtime = 1):
        ### base parameters
        self.pref_dist = 1
        self.L = L
        self.B = B
        ...
        ### there are some different options available, from which you can choose. They are different types of material inclusions
        if self.compute_mat_der:
            ...
            self.alpha_list = [1.2,1.2]### list of design dependent material values
            if self.Friedrich_setup or self.Friedrich_setup_2: 
                self.alpha_fix_list = [1.2,1.2,1.2,1.2] ### sets the initial list for fixed material strength values
            self.num_alpha = 2
    ...
    def full_hessian(self,y,phi,dist_hist,y_1, hist_edges,edge_index,y_full, reshape = True, return_parts = False):
        ### this function is not used anywhere in the code, but it is the full hessian of the coupled system wrt to not just the nodes, but also damage, its REALLY large so not used
        if reshape: 
            z = np.zeros((self.L*self.B*2,))
            z[self.dir_nodes] = y_full[self.dir_nodes]
            z[self.free_nodes] = y
    ...
    def solve(self, method):
        ...
        ### list of jacobians
        self.J_list = []#np.zeros((self.L*self.B*2,))
        self.J_list_alpha = []#np.zeros((self.L*self.B*2,self.num_alpha))
